...
async def extract_pid_tags(file: UploadFile = File(...)) -> dict[str, object]:
    ...
    try:
        ...
        if size_mb > SETTINGS.max_upload_mb:
            raise HTTPException(
                status_code=413,
                detail=f"PDF is {size_mb:.1f} MB. Upload a file smaller than {SETTINGS.max_upload_mb} MB.",
            )
        ...
        if page_count > SETTINGS.max_pages:
            raise HTTPException(
                status_code=413,
                detail=(
                    f"PDF has {page_count} pages. This free Render deployment processes up to "
                    f"{SETTINGS.max_pages} pages per upload. Split the drawing PDF and try again."
                ),
            )
